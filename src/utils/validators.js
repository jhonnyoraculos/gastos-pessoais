...
function validateIncomePayload(payload, { partial = false } = {}) {
  ...
  if (partial && Object.keys(value).length === 0) {
    errors.push('Nenhum campo valido enviado para atualizar ganho.');
  }
...
function validateIncomesQuery(query) {
  const errors = [];
  const value = {};
  ...
  if (query.method !== undefined && query.method !== null && String(query.method).trim() !== '') {
