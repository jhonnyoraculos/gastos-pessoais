...
router.get('/', async (req, res, next) => {
  try {
    const { errors, value } = validateExpensesQuery(req.query);
    if (errors.length) {
    ...
    filters.push(`e.date >= ${addParam(range.start)} AND e.date < ${addParam(range.end)}`);

    if (value.category) {
      if (/^\d+$/.test(value.category)) {
        filters.push(`e.category_id = ${addParam(Number(value.category))}`);
      } else {
    ...
    if (value.type) {
      filters.push(`e.type = ${addParam(value.type)}`);
    }

    if (value.q) {
      const needle = `%${value.q}%`;
      filters.push(`(e.description ILIKE ${addParam(needle)} OR COALESCE(e.notes, '') ILIKE ${addParam(needle)})`);
    }
    ...
    const baseFrom = `
      FROM gp_expenses e
      JOIN gp_categories c ON c.id = e.category_id
      ${whereSql}
    `;
    ...
    res.json({
      items: dataRes.rows.map(mapExpense),
      total,
      limit: value.limit,
      offset: value.offset,
    });
...
router.post('/', async (req, res, next) => {
  try {
    const { errors, value } = validateExpensePayload(req.body, { partial: false });
    if (errors.length) {
      return res.status(400).json({ error: errors.join(' ') });
    }
    ...
    const result = await pool.query(
      `
        WITH inserted AS (
          INSERT INTO gp_expenses (date, amount, description, category_id, type, method, notes)
          VALUES ($1, $2, $3, $4, $5, $6, $7)
          RETURNING id, date, amount, description, category_id, type, method, notes, created_at, updated_at
        )
        SELECT
          i.id,
          i.date,
      ...
      `,
      [value.date, value.amount, value.description, value.category_id, value.type, value.method, value.notes ?? null]
    );
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    const currentRes = await pool.query('SELECT * FROM gp_expenses WHERE id = $1', [expenseId]);
...
router.delete('/:id', async (req, res, next) => {
  try {
    ...
    const result = await pool.query('DELETE FROM gp_expenses WHERE id = $1', [expenseId]);
