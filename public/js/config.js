(() => {
  ...
  async function saveSettings() {
    ...
    if (!Number.isInteger(payload.payday_day) || payload.payday_day < 1 || payload.payday_day > 28) {
      toast('error', 'Dia do pagamento deve ser entre 1 e 28.');
      return;
    }

    if (!Number.isFinite(payload.monthly_budget) || payload.monthly_budget < 0) {
      toast('error', 'Orcamento mensal invalido.');
      return;
    }
  ...
  async function deleteMonthlyIncome() {
    const month = normalizeMonthValue(els.monthlyIncomeMonth.value);
    if (!month) {
      toast('error', 'Informe um mes valido.');
      return;
    ...
    const confirmed = window.confirm(`Remover o salario mensal de ${formatMonthLabel(month)}?`);
    if (!confirmed) return;

    try {
      setLoading(true, 'Removendo registro mensal...');
  ...
  async function loadInitialData() {
    try {
      ...
      await Promise.all([loadSettings({ withLoading: false }), loadMonthlyIncomeForSelectedMonth({ withLoading: false })]);
    } catch (error) {
      toast('error', error.message);
    } finally {
      setLoading(false);
  ...
  function renderSummaryCards() {
    const monthlyBudget = Number(state.settings?.monthly_budget || 0);
